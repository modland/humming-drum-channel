...
function playUserSound(user){ // this adaptation plays client's tracks
	SC.get('/tracks', 
		{user_id:"4065989"},
		function (tracks) {
			alert(tracks.length + " tracks returned")
			...
			SC.oEmbed(tracks[0].uri, {autoplay:true}, document.getElementById('target'));
...
function playTagSound(tag){ // plays the clients tracks filtered by tag
	SC.get('/tracks', 
		{tags: tag}, 
		function (tracks) {
			alert(tracks.length + " tracks returned")
			var random = Math.floor(Math.random() * (tracks.length - 1));
			// TODO: something is producing "TYPE ERROR: tracks[random] is undefined"
